package com.scentsnote.android.viewmodel.signin
...
class SignInViewModel : ViewModel() {
    ...
    fun checkRegisterInfo(){
        if(ScentsNoteApplication.prefManager.haveToken()){
            emailTxt.postValue(ScentsNoteApplication.prefManager.userEmail)
            passwordTxt.postValue(ScentsNoteApplication.prefManager.userPassword)
        }
    }
        ...
        viewModelScope.launch {
            try{
                ...
                signRepository.postLoginInfo(loginInfo).let {
                    ScentsNoteApplication.prefManager.userIdx = it.userIdx
                    ScentsNoteApplication.prefManager.userEmail = emailTxt.value.toString()
                    ScentsNoteApplication.prefManager.userPassword = passwordTxt.value.toString()
                    ScentsNoteApplication.prefManager.userNickname = it.nickname
                    ScentsNoteApplication.prefManager.userGender = it.gender
                    ScentsNoteApplication.prefManager.userAge = it.birth
                    ScentsNoteApplication.prefManager.accessToken = "Bearer "+it.token
                    ScentsNoteApplication.prefManager.refreshToken = "Bearer "+it.refreshToken
